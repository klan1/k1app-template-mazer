...
use k1lib\html\div;
use k1lib\html\DOM;
use k1lib\html\html_document as K1libHtml_document;
use k1lib\html\script;

use const k1app\template\maze\TPL_URL;

class html_document extends DOM
{
    private static div $app;
    private static div $sidebar;
    private static div $main;

    static function hello_world()
    {
        self::main()->append_p('Hello world');
    }

    static function start($lang = 'en'): K1libHtml_document
    {
        $thml_document = parent::start($lang);

        $head = $thml_document->head();
        $body = $thml_document->body();
        ...
        $head->link_css(\k1app\K1APP_BASE_URL)->set_attrib("rel", "canonical");
        ...
        $head->link_css(TPL_URL . "assets/compiled/css/app.css");
        $head->link_css(TPL_URL . "assets/compiled/css/app-dark.css");

        /**
         * HTML BODY
         */
        $body->append_child_head(new script(TPL_URL . "assets/static/js/initTheme.js"));

        self::$app = $body->append_div(null, 'app');
        self::sidebar();
        self::$main = self::$app->append_div(null, 'main');


        $body->append_child_tail(new script(TPL_URL . "assets/static/js/components/dark.js"));
        $body->append_child_tail(new script(TPL_URL . "assets/extensions/perfect-scrollbar/perfect-scrollbar.min.js"));
        $body->append_child_tail(new script(TPL_URL . "assets/compiled/js/app.js"));

        $body->append_child_tail((new div(NULL, "k1lib-output")));

        return $thml_document;
    }

    static public function app()
    {
        if (empty(self::$app)) {
            self::$app = self::html_document()->body()->append_div(null, 'sidebar');
        }
        return self::$app;
    }
    static public function sidebar()
    {
        if (empty(self::$sidebar)) {
            self::$sidebar = self::$app->append_div(null, 'sidebar');
            $sidebar_div = self::$sidebar->append_div("sidebar-header position-relative");
            $sidebar_div
                ->append_div("d-flex justify-content-between align-items-center")
                ->append_div('logo')
                ->append_a('/')
    ...
    {
        if (empty(self::$main)) {
            self::$main = self::$app->append_div(null, 'sidebar');
            $header = new header;
        }
        return self::$main;
    }
